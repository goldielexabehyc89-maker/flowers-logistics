...
План выкатки на production:
 ...
 10. проверить подложку по манифесту и пересчитать на сервере SHA-256 обоих
 ...
 11. доставить Compose-файл выкатываемой версии и сверить его контрольную сумму
 12. поднять маршрутизатор, дождаться набора тайлов и посчитать ПРЕДЕЛЬНУЮ
     матрицу 60×60 обоими профилями плюс регрессию пешеходного профиля
 ...
 15. только после этого применить миграции
 16. перезапустить приложение
 17. дождаться /ready на внешнем порту production и отчитаться о развёрнутом SHA
...
# Отдельно и до приложения — по той же причине, что и маршрутизатор.
