...
План выкатки на staging:
 ...
 11. поднять решатель ОТДЕЛЬНО
 12. проверить решатель пробной задачей с пользовательской матрицей: он обязан
 ...
 13. только после этого применить миграции (prisma migrate deploy)
 14. запустить приложение из точной версии образа
 15. дождаться /ready на внешнем порту staging
 16. записать версию в state/verified-versions и отчитаться о развёрнутом SHA
