...
# Доставляет на сервер Compose-файл выкатываемой версии и сверяет его.
#
...
# с кодом, но до сих пор попадал на сервер только руками при первоначальной
# настройке. Получалось, что выкатка придирчиво сверяет OCI-метку образа —
# и тут же запускает его файлом произвольного возраста.
#
...
#
# Передача через base64 избавляет от зависимости от scp и rsync: у нас уже есть
# рабочее ssh-соединение, а один способ доставки проще проверять, чем два.
sync_compose_file() {
  if is_dry_run; then
    log "сухой прогон: Compose-файл не доставляется (нужен доступ к серверу)"
    return 0
  fi

  local source_file="${REPO_ROOT}/deploy/${COMPOSE_FILE}"
  [ -f "${source_file}" ] || fail "в репозитории нет файла ${source_file}"

  local expected encoded
  expected="$(sha256_of "${source_file}")"
  encoded="$(base64 < "${source_file}" | tr -d '\n')"

  # Запись во временный файл и переименование: прерванная передача не должна
  # оставить на сервере обрезанный Compose-файл вместо рабочего.
  remote "printf '%s' '${encoded}' | base64 -d > '${REMOTE_DIR}/${COMPOSE_FILE}.new' \
    && mv '${REMOTE_DIR}/${COMPOSE_FILE}.new' '${REMOTE_DIR}/${COMPOSE_FILE}'"

  local actual
  actual="$(remote "sha256sum '${REMOTE_DIR}/${COMPOSE_FILE}'" | awk '{print $1}')"

  if [ "${actual}" != "${expected}" ]; then
    fail "Compose-файл на сервере не совпал с версией из репозитория: ${actual} вместо ${expected}"
  fi

  log "Compose-файл доставлен и сверен: ${expected:0:16}…"
}
