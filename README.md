...
**Текущее состояние: этап 1, ветка `feat/stage1-foundation`.** Реализованы фундамент,
схема базы, health/readiness и структурированные логи. Авторизация, интерфейс, realtime
и деплой-каркас идут следующими ветками — см. [ROADMAP.md](ROADMAP.md).
...
- `/` — служебная страница сборки (рабочий интерфейс появится в ветке `feat/stage1-ui-shell`).
...
`test:critical` требует поднятой базы: часть проверок работает с реальной PostgreSQL
и намеренно падает, а не пропускается, если `DATABASE_URL` не задан.
