...
const configSchema = z.object({
  ...
  /**
   * Откуда брать адрес заказа для геокодирования.
   *
   * `shipmentAddress` — строка одним полем, как было всегда. `shipmentAddressFull`
   * собирает адрес из разобранных частей: индекс, страна, регион, город, улица
   * и дом. Квартира, подъезд, этаж и комментарии в него не входят — геокодеру
   * они не помогают, а мешают: он ищет дом, а не квартиру в нём.
   *
   * Значение временное и включается только там, где его проверяют. Умолчание
   * сохраняет прежнее поведение до единого символа.
   */
