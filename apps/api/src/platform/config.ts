...
const configSchema = z.object({
  ...
  /**
   * Pepper кода привязки обработчика печати.
   *
   * ОТДЕЛЬНЫЙ ОТ `AUTH_PIN_PEPPER` намеренно. Это разные контуры: один защищает
   * вход человека, другой — выдачу рабочего места компьютеру. Общий секрет
   * означал бы, что компрометация любого из них обесценивает оба сразу, а
   * плановая смена одного заставляет менять и второй.
   */
  PRINT_AGENT_PAIRING_PEPPER: z
    .string()
    .min(32, 'PRINT_AGENT_PAIRING_PEPPER должен быть не короче 32 символов'),
  // --- МойСклад ---
