...
const configSchema = z.object({
  ...
  /**
   * Граница ВКЛЮЧЕНИЯ автоматического расчёта расстояния за МКАД.
   *
   * Сравнивается с московской календарной датой ДОСТАВКИ заказа. Заказы с датой
   * СТРОГО РАНЬШЕ этой границы автоматика не трогает: обработчик задания и
   * восстановительный проход пропускают их, снимок и проводки не создаются, а
   * старые расстояния и финансовые проводки остаются прежними. Отсутствие
   * переменной ПОЛНОСТЬЮ выключает автоматику: обработчик выходит сразу, ничего
   * не считая и не начисляя (границу держит обработчик, как у florist.dispatch).
   */
  MKAD_DISTANCE_AUTO_CALC_FROM: z
    .preprocess(
      (value) => (typeof value === 'string' && value.trim() === '' ? undefined : value),
      z.string().optional(),
    )
    .refine((value) => value === undefined || isCalendarDate(value), {
      message: 'MKAD_DISTANCE_AUTO_CALC_FROM должен быть московской календарной датой ГГГГ-ММ-ДД',
    }),

  /**
   * Узкая граница ДВУХ рабочих очередей: «Самовывоз → Ожидают выдачи» и
   * «Склад → Ожидают приёмки».
   *
   * Сравнивается с московской календарной датой ДОСТАВКИ заказа. Заказы с датой
   * СТРОГО РАНЬШЕ границы не показываются в этих двух очередях — ни в списках, ни
   * в поиске, ни в счётчиках, ни в фильтрах, ни при пагинации. Заказы БЕЗ даты
   * этой границей не скрываются: они остаются видимыми как требующие разбора.
   * Отсутствие переменной сохраняет прежнее поведение. Это НЕ трогает
   * OPERATIONS_START_DATE и другие рабочие разделы — только две указанные очереди.
   */
  PICKUP_WAREHOUSE_QUEUE_DATE_FROM: z
    .preprocess(
      (value) => (typeof value === 'string' && value.trim() === '' ? undefined : value),
      z.string().optional(),
    )
    .refine((value) => value === undefined || isCalendarDate(value), {
      message:
        'PICKUP_WAREHOUSE_QUEUE_DATE_FROM должен быть московской календарной датой ГГГГ-ММ-ДД',
    }),
  ...
  /**
   * UUID канала продаж, заказы которого исключаются из «Логистика → Сделки».
   *
   * Ровно одна узкая настройка ради исключения канала (например, Flowwow):
   * такие заказы продолжают импортироваться, храниться, открываться напрямую и
   * жить в остальных потоках — их убирают ТОЛЬКО из списка, карты, счётчиков,
   * поиска и массовых действий «Сделок». Канал опознаётся по идентификатору, а
   * не по названию: регистр и текст названия ненадёжны.
   *
   * Не задано — исключение выключено, поведение прежнее. Неверное значение
   * роняет запуск: молча взять не тот UUID опаснее, чем не подняться.
   */
  DEALS_EXCLUDED_SALES_CHANNEL_ID: z
    .preprocess(
      (value) => (typeof value === 'string' && value.trim() === '' ? undefined : value),
      z.string().optional(),
    )
    .refine(
      ...
      { message: 'DEALS_EXCLUDED_SALES_CHANNEL_ID должен быть идентификатором канала продаж' },
    ),

  /**
   * UUID канала продаж Flowwow: его заказы ОПЕРАЦИОННО считаются самовывозом.
   *
   * Отдельная переменная, НЕ связанная с {@link DEALS_EXCLUDED_SALES_CHANNEL_ID}:
   * из «Сделок» в будущем могут исключаться и другие каналы, а семантику
   * самовывоза несёт именно Flowwow. Канал опознаётся ТОЛЬКО по идентификатору
   * (не по названию/регистру/частичному совпадению). Исходный `deliveryMethodId`
   * заказа НЕ переписывается — значение МоегоСклада сохраняется для истории и
   * повторной синхронизации; операционный самовывоз выводится предикатом.
   *
   * Не задано — Flowwow-как-самовывоз выключен, поведение прежнее. Неверное
   * значение роняет запуск.
   */
  MOYSKLAD_FLOWWOW_SALES_CHANNEL_ID: z
    .preprocess(
      (value) => (typeof value === 'string' && value.trim() === '' ? undefined : value),
      z.string().optional(),
    )
    .refine(
      (value) =>
        value === undefined ||
        /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(value),
      { message: 'MOYSKLAD_FLOWWOW_SALES_CHANNEL_ID должен быть идентификатором канала продаж' },
    ),

  /**
   * UUID статуса источника «Новый».
   *
   * Заказы в этом статусе исключаются из ВСЕХ рабочих очередей (Сделки, очередь
   * флориста, «Ожидают выдачи», «Ожидают приёмки») единым серверным предикатом,
   * независимо от способа получения, канала, Flowwow и оплаты. Статус
   * опознаётся только по UUID — название в аккаунте переименовывают, UUID
   * стабилен, и в код он не зашивается.
   *
   * Не задано — исключение выключено, поведение прежнее: приложение стартует
   * штатно и очереди работают как раньше. Неверное значение роняет запуск.
   */
  MOYSKLAD_NEW_STATE_ID: z
    .preprocess(
      (value) => (typeof value === 'string' && value.trim() === '' ? undefined : value),
      z.string().optional(),
    )
    .refine(
      (value) =>
        value === undefined ||
        /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(value),
      { message: 'MOYSKLAD_NEW_STATE_ID должен быть идентификатором статуса заказа' },
    ),
