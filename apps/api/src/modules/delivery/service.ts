 ...
 *
 * Обязателен при ЛЮБОЙ причине, а не только при «Другое» (решение владельца
 * от 17.08.2026). Причина отвечает на вопрос «что за случай», комментарий —
 * «что именно произошло у этой двери»: без него разбирательство с клиентом
 * упирается в восемь одинаковых строк «Нет ответа» без единой подробности.
 *
 * Признак `requiresComment` в справочнике остаётся: он был правилом более
 * слабым, и новое правило его не нарушает.
 */
function assertCommentPresent(
  ...
  if (reason !== null && comment === null) {
    throw new AppError('VALIDATION_FAILED', {
      publicMessage: 'Опишите, что произошло: комментарий обязателен для недоставки.',
    });
  }
}
