 ...
 *
 * Втрое больше периода отклика агента: одна пропущенная отметка бывает
 * из-за сети, три подряд означают, что компьютер выключен.
 */
export const OFFLINE_AFTER_MS = 90_000;

/** Как часто агенту следует отмечаться. Значение уходит агенту в ответе. */
...
export async function recordHeartbeat(
  ...
  await db.printPoint.update({
    ...
    data: {
      lastSeenAt: now,
      ...(error === null ? {} : { lastErrorAt: now, lastErrorText: error }),
    },
  });
}
