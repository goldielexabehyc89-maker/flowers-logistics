...
export const OUTBOX_TOPICS = [
  ...
  'mkad.distance',
  /*
   * Денежные последствия изменений заказа в источнике: рост оплаченной суммы
   * и отмена. Отдельным заданием, а не внутри импорта, потому что импорт
   * блокирует строку ЗАКАЗА, а фиксация доставки — строку МАРШРУТА: writing
   * в журнал прямо из импорта свело бы два разных порядка блокировок.
   */
  'finance.order_sync',
] as const;
