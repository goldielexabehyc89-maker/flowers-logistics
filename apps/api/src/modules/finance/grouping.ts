...
export function groupSettlement(
  ...
  /*
   * Операции доставки показывает её строка — но только если строка есть.
   *
   * Корректировка или отмена может прийти в ДРУГОЙ день: доставка вчера,
   * снятие наличных сегодня. В отчёте за сегодня строки вчерашней доставки нет,
   * и запись пропадала целиком — баланс менялся, а объяснить его было нечем.
   * Поэтому привязанная к доставке запись попадает в журнал дня тогда и только
   * тогда, когда её доставки нет среди строк отчёта. Двойного счёта не
   * возникает: запись учтена либо строкой, либо журналом.
   */
  const attemptsWithRows = new Set(rows.map((row) => row.attemptId));

  for (const entry of entries) {
    if (entry.attemptId !== null && attemptsWithRows.has(entry.attemptId)) {
      continue;
    }
