...
export class AuditSecretLeakError extends Error {
  constructor(field: string) {
    super(`Попытка записать в аудит секретное поле «${field}»`);
    this.name = 'AuditSecretLeakError';
  }
}

function assertNoSecrets(value: Record<string, unknown> | null | undefined): void {
  if (value === null || value === undefined) {
    return;
  }
  for (const key of Object.keys(value)) {
    if (FORBIDDEN_AUDIT_FIELDS.test(key)) {
      throw new AuditSecretLeakError(key);
    }
  }
}
...
export async function writeAudit(tx: TransactionClient, entry: AuditEntryInput): Promise<void> {
  assertNoSecrets(entry.oldValue);
  assertNoSecrets(entry.newValue);
