 ...
 *
 * Не только логистике. Подтверждённый лист определяет, что собирать флористу
 * и что комплектовать складу: состав, порядок и курьер меняют их работу так же,
 * как работу логиста. Пока событий не было, обе роли узнавали об изменениях
 * только по F5 — и собирали заказ, уже снятый с маршрута.
 *
 * Курьеру глобальная картина по-прежнему не нужна: свой маршрут он видит
 * событиями доставки. В событии нет ни номера заказа, ни адреса, ни состава —
 * только повод перечитать собственный список.
 */
