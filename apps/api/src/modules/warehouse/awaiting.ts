 ...
 *
 * ПОЧЕМУ ОТДЕЛЬНО УЧИТЫВАЕТСЯ ОТГРУЗКА КУРЬЕРУ.
 *
 * Коробку можно передать курьеру, ни разу не поставив на полку: складская
 * отгрузка по отметке без ячейки и ручная отгрузка логистом. Размещения у
 * такого заказа нет, выдачи покупателю тоже, и он оставался в очереди, хотя
 * физически уехал. Факт отгрузки — это переход листа `CONFIRMED → ACTIVE`;
 * к нему сходятся оба пути, поэтому проверяется он, а не сессия выдачи и не
 * аудит. Само сканирование, открытая или отменённая сессия листа не отгружают
 * и заказ не скрывают.
 *
 * Факт привязан к ТЕКУЩЕМУ кругу сборки временем: отгрузка считается, только
 * если случилась после сборки этого круга. Пересборка участие в листе не
 * снимает — круги разводит сравнение, — и без этого заказ, собранный заново
 * после отгрузки, никогда бы не появился в приёмке. Отмена отгрузки возвращает
 * лист в `CONFIRMED`, и заказ появляется снова: факт больше не действует.
 */
 ...
 * не списан; ТЕКУЩИЙ круг сборки ещё не размещался (нет размещения этого
 * круга — ни активного, ни освобождённого); и текущий круг не отгружен
 * курьеру действующей отгрузкой листа.
 *
 * ОДНО условие на список, поиск, счётчики чипов, бейдж вкладки и страницы:
 * все запросы ниже подставляют этот фрагмент, поэтому разойтись они не могут.
 */
function baseFilter(
  queueDateFrom?: string | undefined,
  newStateId?: string | null | undefined,
  ...
  return Prisma.sql`
    ...
    WHERE o."fulfillmentProcessState" = 'ASSEMBLED'
      ...
      -- Текущий круг сборки УЖЕ отгружен курьеру: актуальное участие в листе,
      -- лист отгружен (отгрузка не отменена), и переход в «отгружен» случился
      -- после сборки этого круга. Складская отгрузка по отметке и ручная
      -- отгрузка логистом сходятся к одному переходу, поэтому проверяется он.
      -- Скан, открытая или отменённая сессия лист не отгружают — заказ виден.
      -- Пересборка после отгрузки даёт более позднюю сборку — заказ виден.
      AND NOT EXISTS (
        SELECT 1
        FROM "RouteOrder" ro
        ...
        WHERE ro."orderId" = o."id"
          ...
          AND o."fulfillmentAssembledAt" IS NOT NULL
          AND EXISTS (
            SELECT 1 FROM "RouteStateTransition" t
            WHERE t."routeId" = r."id"
              AND t."toState" = 'ACTIVE'
              AND t."occurredAt" >= o."fulfillmentAssembledAt"
          )
      )
  `;
}
