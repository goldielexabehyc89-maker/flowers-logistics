 ...
 * Счётчик, а не часы.
 *
 * Прежний вариант брал `hrtime.bigint() % 100_000`: пространство значений — сто
 * тысяч, и внутри файла из полусотни проверок пара точек совпадала примерно
 * в одном прогоне из ста. Совпадение точек означает совпадение `keyHash`,
 * а он уникален — проверка, которая сама вставляет строку под этим ключом,
 * падала с нарушением уникальности на ровном месте. Счётчик исключает
 * совпадение по определению.
 */
let pointsSeed = 0;

function uniquePoints(): LatLon[] {
  pointsSeed += 1;
  const seed = pointsSeed;
  return [
