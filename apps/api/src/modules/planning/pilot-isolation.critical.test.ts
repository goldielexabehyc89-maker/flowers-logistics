 ...
 * проверкой графа, что и операторская команда. Затем по явному перечню
 * продуктовых мест ищется синтетический след пилота, а обе разрешённые
 * технические области — `RouteMatrixCache` и `IntegrationStatus` — проверяются
 * на то, что они действительно заполнены этим прогоном. Любая продуктовая
 * запись ломает тест, и это единственный способ заметить её раньше сервера.
 *
