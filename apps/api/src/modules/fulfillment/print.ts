...
export async function listPrintJobs(
  ...
  const where = {
    state: { in: [...states] },
    /*
     * Переданное принтеру задание из «Требуют внимания» НЕ исключается.
     *
     * Бланк всё ещё ждёт отметки человека: `SENT_TO_PRINTER` означает лишь, что
     * наклейка ушла в спулер сама, а не что бумага вышла и её кто-то проверил.
     * Пока состояние остаётся `PENDING`, задание обязано быть на глазах — со
     * своей отметкой «Передано принтеру», кнопками просмотра, PDF, этикетки,
     * повторной печати и подтверждения. Из внимания оно уходит только по
     * человеческому подтверждению, когда `state` становится `PRINTED`
     * (а такого состояния в наборе `attention` уже нет).
     *
     * Прежнее исключение `deliveryState = SENT_TO_PRINTER` прятало эти задания
     * (например 141628A) и оставляло автоматическую печать без человеческого
     * контроля выхода бумаги.
     */
    ...(general
...
export async function retryPrint(
  ...
  const created = await db.$transaction(async (tx) => {
     ...
     *
     * Точка берётся из активной смены ИСПОЛНИТЕЛЯ по `activeKey` — одинаково
     * для флориста и администратора. Активной смены с точкой нет — повтор
