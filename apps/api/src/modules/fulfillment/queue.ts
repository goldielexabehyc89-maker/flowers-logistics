...
export interface QueueOrder {
   ...
   * заказ не возвращался. Возвращённые встают В НАЧАЛО очереди (выше всех
   * обычных, самовывозов и листов), между собой — по времени возврата. Один
   * ключ читают и список, и авто-раздача. Маркер снимается, когда заказ
   * уходит из свободной очереди в сборку (взятие/автораздача/переназначение):
   * приоритет относится к этому возврату, а не к будущим циклам.
   */
...
export function sortQueue(
  ...
  return [...orders].sort((a, b) => {
    /*
     * Возвращённые из карантина «Нет цветов» — в НАЧАЛО очереди, ВЫШЕ всего
     * остального (включая ближайшие самовывозы и маршрутные листы).
     *
     * Ответственный вернул такой заказ осознанно: за ним стоит выясненная
     * причина простоя, и следующий свободный флорист обязан взять именно его,
     * а не рядовой заказ. Приоритет действует лишь на СЛЕДУЮЩУЮ выдачу
     * свободного заказа — уже назначенную работу он не трогает. Между собой
     * возвращённые идут по времени возврата (кого раньше вернули — выше), при
     * равном времени добор устойчив общими правилами ниже. Один и тот же ключ
     * читают и этот список, и авто-раздача.
     */
