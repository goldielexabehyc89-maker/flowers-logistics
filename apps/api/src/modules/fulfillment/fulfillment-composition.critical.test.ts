...
describe('единица измерения количества', () => {
  ...
  const unitRows = [
    ...
    // У этой единицы обозначения нет вовсе — остаётся только название, и
    // сокращает его утверждённая владельцем таблица.
    { id: METRE, name: 'Метр' },
    // А эту сокращать нечем: в таблице её нет, и придумывать сокращение
    // запрещено — на бланке догадка неотличима от факта.
  ...
  it('единица приходит ссылкой, а название — одним справочником на весь проход', async () => {
    ...
    const order = orderRow({
      positions: [
        ...
        // Единица, которую сокращать нечем: показывается как в справочнике.
        positionRow({ quantity: 1, name: 'Коробка', uomId: PACK }),
      ],
    });
    ...
    // ЧЕЛОВЕКУ УХОДИТ КОРОТКОЕ ОБОЗНАЧЕНИЕ.
    //
    // «шт» пришло полем `description` справочника — это настоящее обозначение
    // МоегоСклада. «м» получено утверждённым сокращением полного названия:
    // короткого поля у этой единицы нет, и без сокращения рядом с числом встал
    // бы «Метр». «Упаковка» осталась как есть — сокращать её нечем.
    // Отсутствующая единица остаётся пустой: подставленного «шт» здесь нет.
    expect(positions.map((p) => p.uomName)).toEqual(['шт', 'м', 'шт', null, 'Упаковка']);
    expect(positions.map((p) => p.uomId)).toEqual([PIECE, METRE, PIECE, null, PACK]);
    ...
    expect(components.map((c) => c.uomName)).toEqual(['шт', 'м']);
