...
async function confirmed(
  ...
  // Уже ВЫДАННЫЙ покупателю заказ из «собран» в «требует проверки» НЕ
  // переводится: изменение состава после выдачи не должно возвращать заказ в
  // производственную работу или создавать флористу задачу. Изменение и история
  // при этом сохраняются (ревизия записана выше, уведомление — ниже, но уже как
  // информирование, без «На пересборку»). Признак — факт выдачи, не статус
  // источника. Импортная транзакция уже держит строку заказа, поэтому здесь
  // достаточно чтения без отдельной блокировки.
  const issued = (await tx.orderPickupIssue.count({ where: { orderId: order.id } })) > 0;
  if (order.fulfillmentProcessState === 'ASSEMBLED' && !issued) {
    await tx.deliveryOrder.update({
      where: { id: order.id },
      data: {
