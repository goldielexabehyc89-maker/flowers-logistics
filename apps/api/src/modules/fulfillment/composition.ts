...
/**
 * Утверждённое владельцем сокращение полного названия единицы.
 *
 * Порядок именно такой: сначала берётся НАСТОЯЩЕЕ короткое обозначение
 * справочника МоегоСклада (`uom.description`, см. `MOYSKLAD_MAPPING.md` §13в),
 * и только когда его нет — в дело идёт эта таблица. Она закрывает единственный
 * наблюдённый случай: у единицы заполнено лишь полное название, запасной путь
 * берёт его, и рядом с числом встаёт «штука» вместо «шт».
 *
 * Таблица КОРОТКАЯ намеренно. Владелец утвердил ровно две единицы, потому что
 * только для них сокращение — факт, а не догадка. «Упаковка», «набор», «букет»
 * сюда не добавляются ни сейчас, ни позже: сокращение, придуманное кодом,
 * выглядит на бланке ровно так же достоверно, как пришедшее из справочника,
 * и однажды флорист отмерит не то. Всё, чего здесь нет, показывается как есть.
 *
 * Точка снимается вместе с сокращением: «шт.» и «шт» — одно обозначение,
 * и две записи одного и того же в списке состава читаются как разные единицы.
 */
const APPROVED_SHORT_UNITS = new Map<string, string>([
  ['штука', 'шт'],
  ['штуки', 'шт'],
  ['штук', 'шт'],
  ['шт.', 'шт'],
...
/**
 * Обозначение единицы в том виде, в каком его видит человек.
 *
 * Применяется ДВАЖДЫ и намеренно: на границе импорта — чтобы новые снимки
 * хранили короткое обозначение, — и на проекции карточки и бланка — чтобы
 * заказы, прочитанные прошлыми проходами, показывали его без миграции и без
 * ожидания повторного импорта. Функция идемпотентна: «шт» остаётся «шт»,
 * поэтому двойное применение ничего не меняет.
 */
export function shortUnitLabel(label: string | null): string | null {
  if (label === null) {
    return null;
  }
  ...
  return APPROVED_SHORT_UNITS.get(trimmed.toLowerCase()) ?? trimmed;
}
