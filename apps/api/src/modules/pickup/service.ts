...
function assertIssuable(order: LockedOrder): void {
  ...
  // Пропавший и архивный источник — повод остановиться и разобраться:
  // такой заказ мог быть отменён, и отдавать его нельзя.
    ...
    return await deps.db.$transaction(async (tx: TransactionClient) => {
      ...
      if (placement === null) {
        throw new AppError('CONFLICT', {
          ...
          publicMessage: 'Заказ не принят на склад: фактической ячейки нет.',
      ...
      await writeAudit(tx, {
        ...
        newValue: { orderId: order.id, placementId: placement.id, cellId: placement.cell.id },
        ip: context.ip,
