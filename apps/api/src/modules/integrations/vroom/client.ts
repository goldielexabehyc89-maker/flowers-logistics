...
const stepSchema = z.object({
  ...
  /**
   * Ожидание на месте после прибытия, в секундах.
   *
   * Без него проверить обещание клиенту невозможно. По контракту VROOM
   * `time_windows` ограничивает НАЧАЛО ОБСЛУЖИВАНИЯ, а не физическое прибытие:
   * приехав раньше, курьер ждёт. Значит доказательством служит сумма
   * `arrival + waiting_time`, а не один `arrival`. Пока это поле отбрасывалось,
   * проверяющий слой сравнивал сырое прибытие с началом окна и объявлял
   * нормальное ожидание нарушением — первый настоящий прогон пилота отказал
   * именно так, на исправных решателе и графе.
   *
   * Отрицательное, нечисловое, NaN и Infinity отвергаются: ожидание такой
   * длины означает, что ответ разобран не тот, и продолжать по нему нельзя.
   */
  waiting_time: z.number().finite().nonnegative().optional(),
});
