 ...
 *
 * Операторская команда запускает СОБРАННЫЙ файл:
 *
 *   npm run moysklad:sync-once
 *
 * В production-образе нет ни TS-исходников, ни `tsx`, ни доступа к реестру npm:
 * команда, ссылавшаяся на исходник, падала там с `tsx: not found`, и первый
 * проход пришлось запускать в обход документации. Поэтому здесь `node` по пути
 * `apps/api/dist`, а исходный вариант для локальной разработки назван отдельно
 * и явно — `npm run moysklad:sync-once:dev`. Скрытого выбора по окружению нет:
 * оператор и разработчик запускают разные, честно названные команды.
 */
