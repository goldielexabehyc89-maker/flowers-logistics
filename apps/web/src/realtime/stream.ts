...
const TOPIC_KEYS: Record<RealtimeTopic, string[][]> = {
   ...
   */
  // Самовывоз показывается сразу после импорта, до приёмки складом, поэтому
  // и создание заказа доходит до раздела выдачи — иначе новая коробка появилась
  // бы у менеджера только по F5.
  'order.created': [...DEALS_SCREEN, ...FLORIST_SCREEN, ...PICKUP_SCREEN, ['status']],
  /*
  ...
  'order.updated': [
    ...
    ...FLORIST_SCREEN,
    // Смена статуса самовывоза (в том числе отмена из источника) обязана
    // убрать или обновить карточку у менеджера без перезагрузки.
    ...PICKUP_SCREEN,
    ['order-window'],
    ['status'],
  ],
  ...
  /*
   * Точка печати: подключилась, отвалилась или сообщила об ошибке.
   *
   * Список точек нужен и администратору в настройках, и флористу при выборе
   * места печати: точка, к которой только что подключили компьютер, обязана
   * появиться в выборе без перезагрузки.
   */
  'print_point.changed': [['print-points'], ['florist-print-points']],
  /*
  ...
  /*
   * Отгрузка листа убирает его заказы из «Ожидают приёмки». Управляющему и
   * менеджеру выдачи складской поток не приходит, поэтому у очереди своя тема:
   * список и бейдж вкладки перечитываются без перезагрузки у всех ролей раздела.
   */
  'warehouse.awaiting_changed': [['warehouse-awaiting']],
