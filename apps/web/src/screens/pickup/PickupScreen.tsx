...
export function PickupScreen(): React.JSX.Element {
  ...
  /** Активная вкладка: «Выдача» (по умолчанию) или «Решения» (карантин «Нет цветов»). */
  const [activeTab, setActiveTab] = useState<'issue' | 'decisions'>('issue');

  // Счётчик открытых карантинов для бейджа вкладки «Решения».
  ...
  return (
    <section className="stack">
      <nav className="wh-tabs" aria-label="Разделы выдачи" data-testid="pickup-tabs">
        <button
          type="button"
          className={
            activeTab === 'issue' ? 'wh-tabs__item wh-tabs__item--active' : 'wh-tabs__item'
          }
          aria-current={activeTab === 'issue' ? 'page' : undefined}
          data-testid="pickup-tab-issue"
          ...
          <div className="card stack">
            ...
            {manualEntry && manualOpen && (
              <div className="stack" data-testid="pickup-manual">
                <Field
                  label="Номер заказа"
                  hint="Найдите заказ и подтвердите выдачу отдельной кнопкой"
                >
                  {(fieldProps) => (
                    <TextInput
                      {...fieldProps}
              ...
              {canIssue(card) && manualEntry ? (
                <div className="row">
                  <Button
                    ...
                    onClick={() =>
                      issue.mutate({ orderNumber: card.orderNumber, source: 'MANUAL' })
                    }
                  >
                    Выдать покупателю
                  </Button>
        ...
        */}
            ...
            {(() => {
              ...
              return next === null ? null : (
                <Button
                  data-testid="pickup-more"
                  onClick={() => setCursors((all) => [...all, next])}
                >
                  Показать ещё
                </Button>
              );
          ...
          {confirm !== null && (
            <ConfirmDialog
              ...
              description={
                confirm.kind === 'issue' ? (
                  <>
                    Заказ <strong>{confirm.card.orderNumber}</strong> будет отмечен выданным
                    покупателю. Действие необратимо.
                  </>
                ) : (
                  <>
                    Самовывоз заказа <strong>{confirm.card.orderNumber}</strong> будет убран из
                    очереди локально. Сам заказ, его данные и история сохраняются, статус в
                    источнике не меняется.
                  </>
                )
              }
